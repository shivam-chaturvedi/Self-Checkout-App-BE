...
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http.csrf(csrf -> csrf.disable()).cors(cors -> cors.configurationSource(corsConfigurationSource())) // Configure
																											// CORS as
																											// per your
																											// need
				.authorizeHttpRequests(auth -> auth
						.requestMatchers("/signup", "/login", "/verify-token", "/product/qr/**", "/store-cart/qr/**",
								"/store-cart/get-attached-user/**", "/cart", "/reports/**", "/notify-threat",
								"/user-cart/add-item-by-rfid-tag/**","/user-cart/get-bill/*")
						.permitAll() // Allow signup/login without authentication
						.requestMatchers("/admin/**").hasRole("ADMIN").requestMatchers("/css/**").permitAll()
